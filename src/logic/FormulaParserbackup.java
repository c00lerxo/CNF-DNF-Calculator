...
public class FormulaParserbackup {
    
    private HashMap SubformulaHashMap = new HashMap();
    private TreeSet VariablesSet = new TreeSet();
   ...
   public int getLastSubformulaIndex() throws NullPointerException {
    ...
    private String parseByOperators(String formula, String operator, int x, int y) {
        ...
        else
        {
            ...
            /* Else block is made to check, if -> (implication) sign wasn't confused with <-> (equivalence) sign. Probably could be done a lot better */
           /* else
            {
                System.out.println("Witamy w bloku else!");
                try
                {
                    position = formula.indexOf(operator, position+1);
                    System.out.println("Znaleziono operator " + operator + "na pozycji" + position);
                    formula = changeFormula(formula, position, position, x, y+1);
                }
                catch (Exception e)
                {
                    System.out.println("Wyjatek w linii 142");
                    System.out.println(e);
                }
    ...
    private String changeFormula(String formula, int left, int right, int x, int y) {
       ...
       /* if(Character.isLetter(formula.charAt(left+x)))
        ...
        if(Character.isLetter(formula.charAt(right+y)))
        {
       ...
       /* Block to check, if the number corresponding to subformula has two digits */
            ...
            System.out.println(formula.charAt(left));
            System.out.println(formula.charAt(right+y));
            System.out.println(formula.charAt(right+y+1));
            System.out.println(formula.charAt(right+y+2));
